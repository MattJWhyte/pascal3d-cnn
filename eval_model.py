...
for cat in ["aeroplane", "bicycle", "bus", "car", "chair", "motorbike"]:
    ...
    shapenet_acc = eval.get_accuracy_vector("models/pascal3d-vp-cnn-vgg_pose-{}-shapenet.pth".format(cat), vgg_pose, cat)
    print("Shapenet : {}".format(np.average(shapenet_acc)))
    np.save("results/vgg_pose-{}-shapenet-acc.npy".format(cat), shapenet_acc)
    pascal_acc = eval.get_accuracy_vector("models/pascal3d-vp-cnn-vgg_pose-{}-pascal.pth".format(cat), vgg_pose, cat)
